...
import { processOrder } from '../../redux/cartSlice';

export const Order = () => {
  const processModalRef = useRef(null);
  const isOrderInProcess = useSelector(state => state.cart.isOrderInProcess);
  ...
  const handleClick = e => {
    ...
    if (target === processModalRef.current) {
  ...
  if (!isOrderInProcess) return null;
  ...
  return (
    <div className="order" ref={processModalRef}>
      <div className="order__wrapper">
